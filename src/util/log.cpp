...
int Logger::open(const char *filename, int level, bool is_threadsafe, uint64_t rotate_size){
    ...
    if(strcmp(filename, "stdout") == 0){
        ...
        if(ret == -1){
            ...
            stats.w_curr = st.st_size;//文件字节数,off_t st_size;//也可理解为偏移量
        }
    }
...
inline static
const char *get_level_name(int level){//输出log会后的头 [FATAL ]:....
    switch(level){
        case Logger::LEVEL_FATAL:
            return "[FATAL] ";
        case Logger::LEVEL_ERROR:
...
int Logger::logv(int level, const char *fmt, va_list ap){
    if(logger.level_ < level){//log的level要比level_高（含义上）才打印。即如果level_为fatal，level为debug，则不打印log
        return 0;
    }        
    ...
    int space = sizeof(buf) - (ptr-buf) - 10;//剩余的空间,输出log的具体内容，整体格式是 时间，logname，日志内容,减去10是保留buf的最后是个字节
    ...
    *ptr = '\0';
    ...
    len = ptr - buf;
    if(this->mutex){
        pthread_mutex_lock(this->mutex);//向文件写buf，要获取锁    
    }
    fwrite(buf, len, 1, this->fp);
    fflush(this->fp);

    stats.w_curr += len;
    stats.w_total += len;//w_total记录所有的字节，w_curr记录文件的off_t
